...
- Entités et effets fréquents : passer par les pools existants (`Spawn/EnemyPool.cs`, `Combat/CombatPools.cs` avec `NodePool<T>`), jamais `QueueFree` + `Instantiate` en rafale. Un objet poolé se réinitialise dans sa méthode de lancement (`Launch`/`Play`), pas dans `_Ready`.
