...
OUTPUT=$(realpath -m "${1:?répertoire de résultats requis}")
shift
SEEDS=("$@")
...
for seed in "${SEEDS[@]}"; do
    timeout 600 "$GODOT" --path . --windowed --resolution 1280x720 --rendering-method gl_compatibility --audio-driver Dummy \
