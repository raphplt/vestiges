...
def build(skeleton: Skeleton) -> list[Part]:
    ...
    parts = [
        ...
        Part(lambda p: capsule(p, s.on_torso("neck", (1.6, -1.0, 3.4)), s.on_torso("neck", (2.4, -6.5, 4.2)), 1.3, 1.1), SCARF),
        ...
        Part(lambda p: capsule(p, s.on_head((0, 0.6, -5.2)), s.on_head((0, -3.2, -7.0)), 1.9, 1.0), HOOD),
        ...
        Part(lambda p: capsule(p, s.on_torso("chest", (3.0, 12.0, -8.8)), s.on_torso("chest", (5.6, 19.0, -9.8)), 0.9), ACCENT),
        Part(lambda p: rounded_box(p, s.on_torso("chest", (5.8, 19.2, -9.8)), (1.8, 0.9, 0.9), 0.3, torso_rotation), METAL),
        # Bretelles.
...
ANIMATIONS = humanoid_animations(Gait())
